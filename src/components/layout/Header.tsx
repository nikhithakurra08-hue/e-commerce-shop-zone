...
import { ShoppingCart, Heart, Search, Sun, Moon, Menu, X, User, Package, LogOut, Settings, ChevronDown } from 'lucide-react'
    ...
    <header className="sticky top-0 z-50 shadow-md">
      ...
      <div className="bg-navy-800 text-white px-4 py-2.5">
        <div className="max-w-7xl mx-auto flex items-center gap-3">
          {/* Logo */}
          <Link to="/" className="flex-shrink-0 text-2xl font-bold text-amazon-500 mr-4 tracking-tight">
            ShopZone
          </Link>
          ...
          <div className="flex items-center gap-1 ml-3">
              ...
              {userDropdown && (
                <div className="absolute right-0 top-full mt-1 w-52 bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 py-2 animate-slide-down z-50">
                  {isAuthenticated ? (
                    ...
                    <>
                      <Link to="/login" onClick={() => setUserDropdown(false)} className="block px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 text-sm font-medium text-amazon-600">Sign In</Link>
                      <Link to="/signup" onClick={() => setUserDropdown(false)} className="block px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 text-sm text-gray-700 dark:text-gray-300">Create Account</Link>
                    </>
                  )}
                </div>
              )}
            ...
            {/* Cart */}
            <Link to="/cart" className="relative p-2 hover:bg-navy-600 rounded-lg transition-colors">
      ...
      <div className="bg-navy-600 text-white text-sm hidden md:block">
        <div className="max-w-7xl mx-auto px-4 flex items-center gap-1 py-1.5 overflow-x-auto">
          ...
          <Link to="/products?featured=true" className="px-3 py-1 hover:bg-navy-500 rounded whitespace-nowrap text-amazon-400 font-medium">
            Today's Deals
          </Link>
        </div>
      </div>
